<div class="container mt-14 flex flex-col md:grid md:grid-cols-4 gap-6">
    <div class="flex flex-col gap-4 w-full col-span-3">
        <div class="flex flex-col w-full rounded-md border-2 border-background-secondary">
            <h1 class="text-3xl font-bold p-4">{{ $product->name }}</h1>

            <!-- Full-width border here -->
            <div class="flex flex-row gap-4 pt-2 border-t border-background-secondary w-full p-4">
                @if ($product->image)
        ...
        <div class="flex w-full rounded-md border-2 border-background-secondary">
            <div class="flex flex-col gap-8 border-t border-background-secondary w-full p-6">
                @if ($product->availablePlans()->count() > 1)
                    <x-form.select wire:model.live="plan_id"
                        class="text-white bg-primary-800 px-2.5 py-2.5 rounded-md w-full" name="plan_id"
                        label="Select a plan">
