<div class="space-y-6 pt-4">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4 border-b border-neutral/10 pb-4 mb-6">
            ...
            <div>
                @if ($service->cancellation && $service->status == 'active')
                    <span class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-full bg-warning/10 text-warning border border-warning/20">
                ...
                @else
                    @php
                        $statusClasses = match($service->status) {
                            ...
                            'suspended' => 'bg-inactive/10 text-inactive border-inactive/20',
                            'cancelled' => 'bg-error/10 text-error border-error/20',
                            default => 'bg-warning/10 text-warning border-warning/20'
                        };
                    @endphp
                    <span class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-full border {{ $statusClasses }}">
                    ...
                    <div class="bg-background rounded-lg border border-neutral/10 divide-y divide-neutral/10">
                        ...
                        @if ($service->plan->type == 'recurring')
                            ...
                            <div class="flex justify-between py-3 px-4">
                                ...
                                <span class="font-medium">{{ $service->expires_at ? $service->expires_at->format('M d, Y') : 'N/A' }}</span>
                            </div>
                        @endif
                    ...
                    <div class="flex flex-wrap gap-3 mb-4">
                        ...
                        @if ($service->cancellable)
                            <x-button.danger class="flex items-center gap-2" wire:click="$set('showCancel', true)">
                                ...
                                <div wire:loading wire:target="$set('showCancel', true)">
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach ($buttons as $button)
                                    @if (isset($button['function']))
                                        <x-button.secondary class="flex items-center gap-2" wire:click="goto('{{ $button['function'] }}')">
                                            {{ $button['label'] }}
                                        </x-button.secondary>
                                    @else
                                        <a href="{{ $button['url'] }}" 
    ...
    @if ($showCancel)
        <x-modal open="true" title="{{ __('services.cancellation', ['service' => $service->product->name]) }}" width="max-w-3xl">
            ...
            <x-slot name="closeTrigger">
                <button wire:click="$set('showCancel', false)" @click="open = false" class="text-base/70 hover:text-base transition-colors duration-200">
    ...
    @if($showCancelUpgrade)
        <x-modal open="true" title="{{ __('services.cancel_upgrade', ['service' => $service->product->name]) }}" width="max-w-3xl">
