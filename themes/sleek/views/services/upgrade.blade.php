                ...
                @if ($service->plan->type == 'recurring')
                    {{ __('services.price_every_period', [
                        'price' => $service->product->price(
                            ...
                            $service->plan->billing_unit,
                            $service->currency_code,
                        ),
                        'period' => $service->plan->billing_period > 1 ? $service->plan->billing_period : '',
                        'unit' => strtolower(
                            trans_choice(__('services.billing_cycles.' . $service->plan->billing_unit), $service->plan->billing_period),
                        ),
                    ]) }}
                @else
...
<div class="flex flex-col gap-2 w-full col-span-1 bg-background-secondary p-3 rounded-md h-fit">
    ...
    <div class="flex flex-row justify-end gap-2 mt-2">
        <x-button.primary class="h-fit"
            :wire:click="($upgradeProduct->upgradableConfigOptions()->count() > 0 && $step == 1)? 'nextStep' : 'doUpgrade'">
