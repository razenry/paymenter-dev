<div class="container mt-14 space-y-4">
    ...
    @forelse ($services as $service)
        <a href="{{ route('services.show', $service) }}" wire:navigate>
            <div
                class="bg-background-secondary hover:bg-background-secondary/90 border border-neutral/20 p-5 rounded-xl mb-4 transition-all duration-200 shadow-sm hover:shadow-md">
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center gap-3">
                        <div class="bg-secondary/10 p-2 rounded-lg">
                            <x-ri-instance-line class="size-5 text-secondary" />
                        </div>
                        <div class="flex flex-col leading-tight max-w-[16rem] sm:max-w-xs md:max-w-sm">
                            <span class="font-medium truncate">
                                {{ $service->product->name }}
                            </span>

                            @if (!empty($service->external_id))
                                <span class="text-xs text-base/50 truncate">
                                    #{{ $service->external_id }}
                                </span>
                            @endif
                        </div>
