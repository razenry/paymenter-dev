                    ...
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-3">
                            <div class="bg-neutral/10 p-2.5 rounded-lg group-hover:bg-primary/10 transition-colors">
                                <x-ri-instance-line class="size-5 text-base/80 group-hover:text-primary transition-colors" />
                            </div>
                            <div>
                                <h3 class="font-semibold text-base group-hover:text-primary transition-colors">
                                    {{ $service->product->name }}
                                </h3>
                                <p class="text-xs text-base/60 flex items-center gap-1.5">
                                    <span>{{ $service->product->category->name }}</span>

                                    @if (!empty($service->external_id))
                                        <span class="text-[10px] px-1.5 py-0.5 rounded bg-neutral/10 text-base/50">
                                            #{{ $service->external_id }}
                                        </span>
                                    @endif
                                </p>
...
@else
    <div class="bg-background-secondary border border-neutral/20 p-6 rounded-xl text-center">
        <div class="flex flex-col items-center justify-center gap-2">
            ...
            <p class="text-xs text-base/60">{{ __('Once you purchase a service, it will appear here for quick access.') }}
            </p>
        </div>
    </div>
@endif